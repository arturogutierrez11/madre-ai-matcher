...
@Injectable()
export class MatchMeliMegatoneCategoriesUseCase {
  constructor(
    @Inject('IMeliCategoriesRepository')
    private readonly meliCategoriesRepository: IMeliCategoriesRepository,

    @Inject('IMegatoneCategoriesRepository')
    private readonly megatoneCategoriesRepository: ICategoriesRepository,

    @Inject('IMadreMegatoneCategoryMatchRepository')
  ...
  async execute() {
    console.log('==============================');
    console.log('Starting ML → Megatone matcher');
    console.log('==============================');
    ...
    const megatoneCategories =
      await this.megatoneCategoriesRepository.getCategoriesTree();

    const total = meliCategories.length;

    console.log(`ML categories loaded: ${total}`);
    console.log(`Megatone categories loaded: ${megatoneCategories.length}`);
    console.log('--------------------------------');
    ...
    for (const meliCategory of meliCategories) {
      ...
      try {
        ...
        await this.madreRepository.save({
          ...
          meliCategoryPath: meliCategory.path,
          megatoneCategoryId: match.categoryId,
          megatoneCategoryPath: match.categoryPath,
        });

        matched++;

        console.log(
          `✔ MATCHED → ML: ${meliCategory.path} → MEGATONE: ${match.categoryPath}`,
        );
      } catch (error) {
        errors++;
    ...
    console.log('==============================');
    console.log('Megatone matcher completed');
    console.log('==============================');
